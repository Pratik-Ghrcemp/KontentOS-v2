...
-- 3. Policy: Authenticated users can upload files to the media bucket
CREATE POLICY "Users can upload media" ON storage.objects
  FOR INSERT TO authenticated
  WITH CHECK (bucket_id = 'media' AND (storage.foldername(name))[1] = auth.uid()::text);

-- 4. Policy: Users can view their own media (and public media if bucket is public, but let's restrict to owner or public)
CREATE POLICY "Public media is viewable by everyone" ON storage.objects
  FOR SELECT USING (bucket_id = 'media');

-- 5. Policy: Users can update their own media
CREATE POLICY "Users can update own media" ON storage.objects
  FOR UPDATE TO authenticated
  USING (bucket_id = 'media' AND (storage.foldername(name))[1] = auth.uid()::text);

-- 6. Policy: Users can delete their own media
